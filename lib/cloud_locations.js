...
function findRandomCloud(highBound, lowBound, callback) {
    console.log('Trying to find cloud...');
    var lon = (Math.random() * (highBound.lon - lowBound.lon)) + lowBound.lon;
    var lat = (Math.random() * (highBound.lat - lowBound.lat)) + lowBound.lat;
    ...
    isCloudAt(loc, function(err, cloudThere) {
        if (err) {
            ...
            findRandomCloud(highBound, lowBound, callback);
        } else {
...
function getCloudLocations(highBound, lowBound, callback) {
    findRandomCloud(highBound, lowBound, function(err, loc) {
        if (err) {
            callback(err);
            return;
        }
